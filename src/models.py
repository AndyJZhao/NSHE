...
class NENS(nn.Module):

    def __init__(self, n, hp):
        super(NENS, self).__init__()
        ...
        self.types = n.node_types
        size = hp.size
        self.t_info = n.t_info
        for t in self.types:
            self.add_module('encoder_' + t, nn.Linear(n.feature[t].shape[1], size['com_feat_dim']))
        ...
        if self.cla_method == '2layer':
            if self.ns_emb_mode == 'TypeLvAtt':
                ...
                self.ns_classifier = NS_MLP_Classifier(len(n.t_info) * emb_dim, emb_dim)
