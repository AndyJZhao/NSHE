...
def run(model, n, optimizer):
    epoch_res_list = []
    for epoch in range(hp.epochs):
        epoch_start_time = time.time()
        n.get_epoch_samples(epoch, hp)
        model.train()  # set paras to train ns_emb_mode
        optimizer.zero_grad()
        node_emb, predict = model(n.adj, n.feature, n.ns_instances)
        pairwise_loss = cal_pairwise_ce_loss(node_emb, n.edge, n.neg_edge)
        cla_loss = cal_cla_loss(predict, n.ns_label)
        # loss = cla_loss
...
def main(hp):
    ...
    node_emb = run(model, n, optimizer)  # 模型训练
    print("Train finished")
    model.eval()
    evaluate(node_emb, n.t_info)
